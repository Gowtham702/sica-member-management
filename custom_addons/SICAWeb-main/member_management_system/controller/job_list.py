...
class JobList(http.Controller):
    ...
    def active_job_list(self, **kw):
        try:
            ...
            JobProvider = request.env['member.job.provider'].sudo()
            JobSeeker = request.env['member.job.seeker'].sudo()

            jobs = JobProvider.search([])
            result = []

            for rec in jobs:
                # Build a single domain:
                # till_date <= required_till  AND (skill overlap OR medium match OR post match)
                domain = []
                if rec.required_till:
                    domain += [('till_date', '<=', rec.required_till)]
                # At least one of these should match
                domain += ['|', '|']
                domain += [('skill_ids', 'in', rec.skill_ids.ids)]  # any overlap
                domain += [('medium_id', '=', rec.medium_id.id or False)]
                domain += [('post_applying_id', '=', rec.post_required_id.id or False)]

                # We only need to know if any exists → search_count is optimal
                is_applied = bool(JobSeeker.search_count(domain))
                ...
                result.append({
                    ...
                    "membership_number": rec.membership_no,
                    "designation": rec.grade,
                    "post_required": rec.post_required_id.name or False,
                    "medium": rec.medium_id.name or False,
                    "experience": rec.experience,
                    "skills": rec.skill_ids.mapped('name'),
                    "project_requirements": rec.project_requirement.mapped('name'),
                    "available_start_date": rec.required_from and fields.Date.to_string(rec.required_from) or False,
                    "available_end_date": rec.required_till and fields.Date.to_string(rec.required_till) or False,
                    "note": rec.note,
                    "state": rec.state,
        ...
        try:
            ...
            for rec in records:
                result.append({
                    "id": rec.id,
                    "name": rec.name,
                    "member_name": rec.member_name or False,
                    "member_id": rec.member_id.id or False,
                    "membership_number": rec.membership_no or False,
                    "post_required": rec.post_required_id.name or False,
                    "medium": rec.medium_id.name or False,
                    ...
                    **({"experience":"Active"} if rec.experience is False else {"experience": rec.experience}),
                    ...
                    "available_start_date": rec.required_from.strftime('%Y-%m-%d') if rec.required_from else None,
                    "available_end_date": rec.required_till.strftime('%Y-%m-%d') if rec.required_till else None,
    ...
    @http.route('/api/get/allgrades', type='json', auth='none', methods=['GET'], csrf=False)
    def get_all_grades(self, **kw):
        print('kw', kw)
        try:
            api_key = kw.get('api_key')
            stored_api_key = "8f4f506e4b4022e154ac3651f9ee006e9b751261"
            ...
            return {
                "status": 200,
                "data": [{'grade': ['Junior', 'Staff', 'Life', 'Active', 'Associate']}]
            }
