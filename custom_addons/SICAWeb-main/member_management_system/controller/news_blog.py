...
class NewsBlog(http.Controller):
    ...
    def action_get_new_talk_details(self, **kw):
        ...
        tech_talk_vals = []
        tech_talk_ids = request.env['tech.talk'].sudo().search([
    ...
    def action_get_sica_blog_details(self, **kw):
        ...
        blog_vals = []
        blog_ids = request.env['sica.blog'].sudo().search([
            ('create_date', '>=', yesterday),
            ('create_date', '<=', today)
        ], order="sequence asc")
    ...
    def action_get_sica_news_details(self, **kw):
        ...
        news_vals = []
        news_ids = request.env['sica.news'].sudo().search([
            ('create_date', '>=', yesterday),
            ('create_date', '<=', today)
        ], order="sequence asc")
    ...
    def action_get_sica_features_details(self, **kw):
        ...
        blog_vals = []
        blog_ids = request.env['sica.features'].sudo().search([
            ('create_date', '>=', yesterday),
            ('create_date', '<=', today)
        ], order="sequence asc")
    ...
    def increase_blog_view(self, **kw):
        ...
        blog = request.env['sica.blog'].sudo().browse(blog_id)

        if not blog.exists():
            return {'success': False, 'message': 'Blog not found'}

        blog.sudo().write({'views': blog.views + 1})
    ...
    def increase_tech_talk_view(self, **kw):
        ...
        tech_talk = request.env['tech.talk'].sudo().browse(tech_id)

        if not tech_talk.exists():
            return {'success': False, 'message': 'Tech talk not found'}

        tech_talk.sudo().write({'views': tech_talk.views + 1})

        return {
            'success': True,
            'message': 'Tech talk count updated successfully',
            'tech_id': tech_talk.id,
    ...
    def increase_news_view(self, **kw):
        ...
        news = request.env['sica.news'].sudo().browse(news_id)

        if not news.exists():
            return {'success': False, 'message': 'News not found'}

        news.sudo().write({'views': news.views + 1})
